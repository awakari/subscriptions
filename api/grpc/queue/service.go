...
type Service interface {
	...
	GetLength(ctx context.Context, name, subj string) (l uint32, err error)
}
...
func (svc service) GetLength(ctx context.Context, name, subj string) (l uint32, err error) {
	var resp *GetLengthResponse
	resp, err = svc.client.GetLength(ctx, &GetLengthRequest{
		Name: name,
		Subj: subj,
	})
	if resp != nil {
